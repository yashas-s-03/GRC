...
function PricingSection() {
  ...
  const tiers = [
    ...
    {
      name: 'Founder', price: '₹1499', period: '/month',
      popular: true,
    ...
    {
      name: 'Team', price: '₹3999', period: '/month',
      popular: false,
...
export default function LandingPage({ setScannedUrl }) {
  ...
  const [url, setUrl] = useState(isDemo ? 'https://stripe.com' : '')
  const [urlError, setUrlError] = useState(null)
  
  const [scanCount, setScanCount] = useState(543)
  useEffect(() => {
    const saved = localStorage.getItem('tl_scan_count')
    if (saved) setScanCount(parseInt(saved, 10))
    else localStorage.setItem('tl_scan_count', '543')
  }, [])
  ...
  function validateURL(input) {
    ...
    try { const p = new URL(u); if (!p.hostname.includes('.')) return 'Please enter a valid domain'; return null }
  ...
  return (
    <div>
      ...
      <section style={{ padding: '5rem 1.5rem 4rem', maxWidth: '1200px', margin: '0 auto', textAlign: 'center' }}>
        ...
        {/* URL Input */}
        <div style={{ maxWidth: '600px', margin: '0 auto 0.75rem' }}>
          <div style={{
            display: 'flex', gap: '0.75rem', background: '#12121a',
            border: `1px solid ${urlError ? '#ef444460' : '#2e2e3e'}`, borderRadius: '1rem', padding: '0.5rem',
            ...
            <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
              ...
              <input
                type="url" value={url}
                onChange={e => { setUrl(e.target.value); if (urlError) setUrlError(validateURL(e.target.value)) }}
            ...
            <button className="btn-primary" onClick={handleScan} id="hero-scan-btn"
              ...
              <Search size={15} /> {isDemo ? 'Demo Scan' : 'Scan My Website'}
            </button>
          </div>
          {urlError && <p style={{ color: '#ef4444', fontSize: '0.78rem', marginTop: '0.5rem', textAlign: 'left', paddingLeft: '0.5rem' }}>{urlError}</p>}
          {isDemo && <p style={{ color: '#f59e0b', fontSize: '0.78rem', marginTop: '0.5rem', textAlign: 'left', paddingLeft: '0.5rem' }}>🎯 Demo mode — pre-filled with stripe.com for a great demo result</p>}
        </div>

        {/* Trust Badges */}
        <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          {[
            { icon: <Users size={14} />, text: `${scanCount} startups scanned` },
            { icon: <Shield size={14} />, text: '16 security checks' },
